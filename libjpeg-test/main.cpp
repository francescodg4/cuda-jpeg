...
#define OUTPUT_BUF_SIZE  ((size_t)(10 * 1024))

METHODDEF(void)
init_destination (j_compress_ptr cinfo)
{
  buffer_destination_mgr *dest = (buffer_destination_mgr *) cinfo->dest;
...
empty_output_buffer (j_compress_ptr cinfo)
{
  ...
  char filename[256];
  snprintf(filename, 256, "/tmp/output-%d.bin", dest->index);
	  
  FILE *outfile = fopen(filename, "wb");

  // if (JFWRITE(outfile, dest->buffer, OUTPUT_BUF_SIZE) !=
  //     (size_t) OUTPUT_BUF_SIZE)
  // 	  printf("error in output_buffer\n");
	  
	
  JFWRITE(outfile, dest->buffer, datacount);

  fclose(outfile);

  printf("datacount %ld, %ld\n", datacount, OUTPUT_BUF_SIZE);
...
GLOBAL(void)
set_buffer_as_dest(j_compress_ptr cinfo, unsigned char * buffer)
{
	buffer_destination_mgr *dest;
	...
	dest = (buffer_destination_mgr *) cinfo->dest;
	...
	int i)
{
	...
	((buffer_destination_mgr *)cinfo->dest)->index = i;
...
/*     } */
	...
	set_buffer_as_dest(&cinfo, buffer);
...
/*     } */
    ...
    FILE *instream = fopen("testimg.bin", "rb");
#endif
    ...
    ((buffer_destination_mgr *) cinfo.dest)->index = 0;
    cinfo.dest->empty_output_buffer(&cinfo);
...
/*     } */
    ...
    // for (i = 0; i < 19; i++) {
    // 	    std::thread t1([] (auto cinfo, auto line, auto v_max_samp) {
    // 			    jpeg_write_raw_data(
    // 				    &cinfo,
    // 				    line,
    // 				    v_max_samp * DCTSIZE);
    // 			    cinfo.dest->empty_output_buffer(&cinfo);
    // 		    }, &cinfo, line, v_max_samp);
   // 	    t1.join();
    // }

    std::thread ts[19];
    unsigned char **lines[19][3];

    for (i = 0; i < 19; i++) {
	    for (k = 0; k < channels; k++)
	    {
		    lines[i][k] = (unsigned char **) malloc(v_max_samp * DCTSIZE * sizeof(unsigned char *));
    ...
    // foo(&cinfo, lines[0], v_max_samp);
    // foo(&cinfo, lines[1], v_max_samp);
    
    for (int i = 0; i < 19; i++) {
    	    ts[i] = std::thread(foo, &cinfo, lines[i], v_max_samp, i + 1);
	    // ts[i].join();
    }
    ...
    ((buffer_destination_mgr *) cinfo.dest)->index = 20;
